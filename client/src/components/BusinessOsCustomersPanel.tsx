...
type DetailState = "idle" | "loading" | "ready" | "auth" | "error";
type InteractionDraft = { interactionType: "NOTE" | "CALL" | "VISIT" | "FOLLOW_UP"; note: string };

const dateFormatter = new Intl.DateTimeFormat("ar-EG", { dateStyle: "medium" });
const numberFormatter = new Intl.NumberFormat("ar-EG");

function formatDate(value: number) {
  return Number.isFinite(Number(value)) ? dateFormatter.format(new Date(Number(value))) : "—";
}

function formatMoney(cents: number, currency: string) {
  return `${numberFormatter.format((Number(cents) || 0) / 100)} ${currency || "EGP"}`;
}

const errorMessage = (error: unknown, fallback = "تعذر الوصول إلى بيانات العملاء.") => {
  const status = (error as { status?: number }).status;
  if (status === 401) return "سجّل الدخول لقراءة بيانات العملاء.";
  if (status === 403) return "لا تملك صلاحية قراءة أو إدارة هذا الجزء من ملف العميل.";
...
export function BusinessOsCustomersPanel({ headers }: Props) {
  ...
  const [interaction, setInteraction] = useState<InteractionDraft>({ interactionType: "NOTE", note: "" });
  const [tagName, setTagName] = useState("");
  const [mutation, setMutation] = useState<"idle" | "loading">("idle");

  const refresh = async (searchTerm = query) => {
    setState("loading");
    setMessage("");
    try {
      setCustomers(await loadBusinessOsCustomers(headers, searchTerm));
      setState("ready");
    } catch (error) {
      const status = (error as { status?: number }).status;
  ...
  useEffect(() => {
    void refresh("");
  }, [headers.authorization, headers["x-tenant-id"]]);

  const visibleCustomers = useMemo(() => {
    const normalized = query.trim().toLocaleLowerCase();
    if (!normalized) return customers;
    return customers.filter((customer) =>
      `${customer.name} ${customer.phone ?? ""} ${customer.email ?? ""}`.toLocaleLowerCase().includes(normalized),
    );
  }, [customers, query]);
  ...
  const openCustomer = async (customerId: string) => {
    ...
    try {
      ...
      const status = (error as { status?: number }).status;
      setDetailState(status === 401 || status === 403 ? "auth" : "error");
      setMessage(errorMessage(error));
    }
  };
  ...
  const submitCreate = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const created = await createBusinessOsCustomer(
        { name: createValues.name.trim(), phone: createValues.phone.trim() || undefined, email: createValues.email.trim() || undefined },
        headers,
      );
  ...
  const submitInteraction = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (!selected || !interaction.note.trim()) {
      setMessage("نوع التفاعل والملاحظة مطلوبان.");
      return;
    }
    setMutation("loading");
    try {
      ...
      setMessage(errorMessage(error, "تعذر حفظ التفاعل."));
    } finally {
      setMutation("idle");
    }
  };
  ...
  const submitTag = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setMessage(errorMessage(error, "تعذر حفظ الوسم."));
    } finally {
      setMutation("idle");
    }
  };
  ...
  if (state === "auth" || state === "error") {
    return <div className="mobile-command-note" role="alert"><b>{message}</b><button className="mobile-command-button" type="button" onClick={() => void refresh()}>إعادة المحاولة</button></div>;
  }
  ...
  return <section className="mobile-plan-card" aria-label="إدارة العملاء الحقيقية">
    ...
    <div className="mobile-command-note">
      <label htmlFor="customer-search">بحث سريع</label>
      <input id="customer-search" aria-label="بحث العملاء" value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void refresh(); }} placeholder="بحث بالاسم أو الهاتف أو البريد" />
    ...
    {!visibleCustomers.length && <div className="mobile-empty"><b>{query.trim() ? "لا توجد نتائج" : "لا يوجد عملاء"}</b><span>{query.trim() ? "جرّب اسماً أو هاتفاً أو بريداً مختلفاً." : "أنشئ أول عميل لتبدأ سجل العلاقة التجارية."}</span></div>}
    {visibleCustomers.map((customer) => <article key={customer.id} className="mobile-data-row">
      <b>{customer.name}</b>
      <span>{customer.phone ?? "بدون هاتف"} · {customer.email ?? "بدون بريد"} · أُضيف في {formatDate(customer.created_at)}</span>
      <button className="mobile-command-button" type="button" onClick={() => void openCustomer(customer.id)} disabled={detailState === "loading"}>{detailState === "loading" ? "جارٍ الفتح..." : "فتح ملف العميل"}</button>
    </article>)}
    ...
    {detailState === "loading" && <div className="mobile-command-note" role="status">جارٍ تحميل تاريخ الطلبات والتفاعلات...</div>}
    {detailState === "error" && <div className="mobile-command-note" role="alert">{message}<button className="mobile-command-button" type="button" onClick={() => detailCustomerId && void openCustomer(detailCustomerId)}>إعادة تحميل الملف</button></div>}
    {selected && detailState === "ready" && <section className="mobile-command-note" aria-label="تفاصيل العميل">
      <button className="mobile-back-button" type="button" onClick={() => { setSelected(null); setDetailCustomerId(null); setDetailState("idle"); }}>إغلاق الملف</button>
      <h3>{selected.customer.name}</h3>
      <p>{selected.customer.phone ?? "بدون هاتف"} · {selected.customer.email ?? "بدون بريد"} · أُضيف في {formatDate(selected.customer.created_at)}</p>
      <h4>سجل الطلبات ({selected.orders.length})</h4>
      {selected.orders.length ? selected.orders.map((order) => <div className="mobile-data-row" key={order.id}><b>{order.state}</b><span>{formatMoney(order.total_cents, order.currency)} · {formatDate(order.created_at)} · {order.id}</span></div>) : <span>لا توجد طلبات مرتبطة بهذا العميل.</span>}
      <h4>التفاعلات ({selected.interactions.length})</h4>
      {selected.interactions.length ? selected.interactions.map((item) => <div className="mobile-data-row" key={item.id}><b>{item.interaction_type}</b><span>{item.note} · {formatDate(item.created_at)}</span></div>) : <span>لا توجد تفاعلات بعد.</span>}
