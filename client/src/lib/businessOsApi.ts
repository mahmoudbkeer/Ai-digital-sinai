...
export async function createBusinessOsSalesReturn(values: { orderId: string; orderItemId: string; productId: string; quantity: number; unitRefundCents: number; reason: string }, headers: Record<string, string>, idempotencyKey = `ui-sales-return-${globalThis.crypto?.randomUUID?.() ?? Math.random().toString(36).slice(2)}`) {
  return productRequest<{ returnId: string; totalCents: number; status: string; replay: boolean }>("/api/platform/returns", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...values, items: [{ orderItemId: values.orderItemId, productId: values.productId, quantity: values.quantity, unitRefundCents: values.unitRefundCents }], idempotencyKey }) }, headers);
}

export async function createBusinessOsInventoryMovement(values: { branchId: string; productId: string; quantityDelta: number; reason: string }, headers: Record<string, string>, idempotencyKey = `ui-inventory-${globalThis.crypto?.randomUUID?.() ?? Math.random().toString(36).slice(2)}`) {
...
export type BusinessOsProfit = { source: string; period: { from: number; to: number }; report: { revenue_cents: number; inventory_cost_cents: number; operating_expense_cents: number; net_profit_cents: number } };
export type BusinessOsReconciliation = { reconciliationId: string; accountCode: string; expectedCents: number; actualCents: number; varianceCents: number; status: "MATCHED" | "VARIANCE" };
export async function loadBusinessOsProfit(headers: Record<string, string>, period: { from: number; to: number }) { const params = new URLSearchParams({ from: String(period.from), to: String(period.to) }); const payload = await productRequest<{ report: BusinessOsProfit["report"]; source: string; period: BusinessOsProfit["period"] }>(`/api/platform/reports/profit?${params}`, {}, headers); return payload as BusinessOsProfit; }
export async function createBusinessOsReconciliation(values: { accountCode: string; expectedCents: number; from: number; to: number; note?: string }, headers: Record<string, string>) { return productRequest<BusinessOsReconciliation>("/api/platform/reconciliations", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(values) }, headers); }
