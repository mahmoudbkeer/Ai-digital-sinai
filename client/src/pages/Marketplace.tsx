...
async function fetchDirectory(query: string, category = "", signal?: AbortSignal) {
  const response = await fetch(`/api/platform/marketplace/directory?query=${encodeURIComponent(query)}&category=${encodeURIComponent(category)}`, { signal, headers: { Accept: "application/json" } });
...
export default function Marketplace() {
  ...
  const [assistantResults, setAssistantResults] = useState<Business[]>([]);
  ...
  useEffect(() => {
    ...
    fetchDirectory(query, category === "الكل" ? "" : category, controller.signal).then((payload) => {
  ...
  const routeAssistantRequest = async (request: string) => {
    const searchQuery = request.trim();
    if (!searchQuery) { setAssistantFeedback("اكتب أو انطق ما تريد الوصول إليه داخل منصتك."); setAssistantResults([]); return; }
    setAssistantState("loading");
    setAssistantFeedback("");
    try {
      const payload = await fetchDirectory(searchQuery);
      const results = payload.businesses ?? [];
      setAssistantResults(results);
      setAssistantState("ready");
      setAssistantFeedback(results.length ? `وجدت ${results.length} نتيجة مطابقة لبحثك.` : "لم أجد نشاطًا مطابقًا، جرّب كلمات أخرى");
    } catch {
      setAssistantResults([]);
      setAssistantState("error");
      setAssistantFeedback("تعذر البحث الآن. حاول مرة أخرى.");
    }
  };

  const openAssistantResult = (business: Business) => {
    setAssistantOpen(false);
    window.requestAnimationFrame(() => {
      document.getElementById(`business-${business.id}`)?.scrollIntoView({ behavior: "smooth", block: "center" });
    });
  };
  ...
  const startVoiceAssistant = () => {
    ...
    recognition.onresult = (event) => { const transcript = event.results[0]?.[0]?.transcript ?? ""; setAssistantQuery(transcript); setListening(false); void routeAssistantRequest(transcript); };
  ...
  return <main dir="rtl" className="marketplace-page">
    ...
    {assistantOpen && <div className="assistant-overlay" role="presentation" onClick={() => setAssistantOpen(false)}><section className="assistant-dialog" role="dialog" aria-modal="true" aria-labelledby="assistant-title" onClick={(event) => event.stopPropagation()}><div className="assistant-dialog-head"><div><span className="assistant-kicker"><Sparkles size={14} /> خدمة AI للنشاط</span><h2 id="assistant-title">ماذا تريد أن تنجز؟</h2></div><button type="button" onClick={() => setAssistantOpen(false)} aria-label="إغلاق المساعد"><X size={19} /></button></div><p>اكتب طلبك أو استخدم صوتك، وسأبحث لك في الأنشطة والخدمات المنشورة فعليًا.</p><form className="assistant-command" onSubmit={(event) => { event.preventDefault(); void routeAssistantRequest(assistantQuery); }}><input value={assistantQuery} onChange={(event) => setAssistantQuery(event.target.value)} placeholder="مثال: صيدلية أو مطعم" aria-label="طلب المساعد الذكي" /><button type="button" className={listening ? "listening" : ""} onClick={startVoiceAssistant} aria-label="التحدث مع المساعد"><Mic size={18} /></button><button type="submit" aria-label="تنفيذ الطلب"><ArrowUpLeft size={18} /></button></form>{assistantState === "loading" && <div className="assistant-feedback" role="status">جارٍ البحث في Marketplace…</div>}{assistantFeedback && assistantState !== "loading" && <div className="assistant-feedback" role="status">{assistantFeedback}</div>}{assistantResults.length > 0 && <div className="assistant-results" aria-label="نتائج بحث المساعد">{assistantResults.map((business) => { const open = isBusinessOpen(business); return <button className="assistant-result" data-business-id={business.id} type="button" key={business.id} onClick={() => openAssistantResult(business)}><span className={`open-status ${open ? "open" : "closed"}`}>{open && <i />} {open ? "مفتوح ومتاح الآن" : "مغلق حاليًا"}</span><strong>{business.name}</strong><small>{business.offering_name || business.subcategory} · {business.district}</small><p>{business.description || "خدمة منشورة في Marketplace."}</p></button>; })}</div>}<div className="assistant-quick-actions"><button type="button" onClick={() => openProgram("market")}><span><Search size={17} /><b>السوق والخدمات</b></span><ChevronLeft size={16} /></button><button type="button" onClick={() => openProgram("work")}><span><BriefcaseBusiness size={17} /><b>مساحة تشغيل نشاطك</b></span><ChevronLeft size={16} /></button><button type="button" onClick={() => openProgram("account")}><span><UserPlus size={17} /><b>حسابك والوصول</b></span><ChevronLeft size={16} /></button></div></section></div>}
