...
test("App Mode opens and navigates across mobile tabs", async ({ page }) => {
  ...
  await page.getByRole("button", { name: /التجارة والعمليات/ }).click();
  await expect(page.getByRole("heading", { name: "التجارة والعمليات" })).toBeVisible();
  ...
  await page.getByRole("button", { name: /العودة إلى وحدات/ }).click();
  await page.getByRole("button", { name: /العملاء إدارة ملفات/ }).click();
  ...
  await page.getByRole("button", { name: /العودة إلى وحدات/ }).click();
  const catalogButton = page.locator("button.mobile-module-card").filter({ hasText: "الكتالوج والخدمات" });
  await catalogButton.scrollIntoViewIfNeeded();
  await catalogButton.click({ position: { x: 16, y: 10 } });
