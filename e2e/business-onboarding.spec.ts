...
test("submits a business from Marketplace work area and keeps it pending review", async ({ page, request }) => {
  ...
  await page.evaluate(({ token, tenantId }) => { localStorage.setItem("platform_token", token); localStorage.setItem("platform_tenant_id", tenantId); }, identity);
  await page.reload();
  ...
  await page.getByLabel("رقم التواصل").fill("201000000001");
  await Promise.all([
    page.waitForResponse((response) => response.url().includes("/api/platform/marketplace/onboarding")),
    page.getByRole("button", { name: "إرسال للمراجعة" }).click(),
  ]);
