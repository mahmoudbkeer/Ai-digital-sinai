...
function writeManifest(output, provider) {
  const sha256 = createHash("sha256").update(readFileSync(output)).digest("hex");
  const manifest = `${output}.sha256.json`;
  writeFileSync(manifest, JSON.stringify({ provider, file: path.basename(output), sha256, bytes: statSync(output).size, createdAt: new Date().toISOString() }) + "\n", { mode: 0o600 });
...
async function postgresBackup() {
  ...
  console.log(JSON.stringify({ status: "COMPLETED", provider: "postgresql", output, ...writeManifest(output, "postgresql"), retention }));
}
...
function sqliteBackup() {
  ...
  const output = path.join(outputDir, `ai-digital-sinai-${stamp}.sqlite`); copyFileSync(source, output); console.log(JSON.stringify({ status: "COMPLETED", provider: "sqlite", output, ...writeManifest(output, "sqlite"), retention }));
}

async function main() {
  if (/^(postgres|postgresql):\/\//i.test(process.env.DATABASE_URL ?? "")) await postgresBackup(); else sqliteBackup();
  const backups = readdirSync(outputDir).filter((file) => file.startsWith("ai-digital-sinai-") && !file.endsWith(".sha256.json")).sort().reverse(); for (const old of backups.slice(retention)) { try { const fs = await import("node:fs/promises"); await fs.unlink(path.join(outputDir, old)); await fs.unlink(`${path.join(outputDir, old)}.sha256.json`).catch(() => {}); } catch { process.exitCode = 1; } }
}
await main();
