...
describe("Business OS workflows", () => {
  ...
  it("supports ledger-linked returns, partial receiving, reconciliation, segments, and detailed reports", async () => {
    ...
    const missingKey = await request(`/api/platform/purchases/${draftPurchaseId}/receipts`, { method: "POST", headers, body: JSON.stringify({ items: [{ purchaseItemId: draftItem.id, quantity: 1 }] }) });
    expect(missingKey.status).toBe(400);
    const overReceive = await request(`/api/platform/purchases/${draftPurchaseId}/receipts`, { method: "POST", headers, body: JSON.stringify({ items: [{ purchaseItemId: draftItem.id, quantity: 2 }], idempotencyKey: "depth-receipt-over" }) });
    expect(overReceive.status).toBe(409);
  ...
  it("proves invoice and expense operational integrity, idempotency, ledger balance, and tenant isolation", async () => {
    const identity = await register("finance-depth@example.com", "Finance Depth Tenant");
    const headers = auth(identity);
    const product = await request("/api/platform/products", { method: "POST", headers, body: JSON.stringify({ businessId: identity.businessId, sku: "FIN-001", name: "منتج مالي", priceCents: 2400 }) });
    const { productId } = await product.json() as { productId: string };
    const supplier = await request("/api/platform/suppliers", { method: "POST", headers, body: JSON.stringify({ businessId: identity.businessId, name: "مورد مالي" }) });
    const { supplierId } = await supplier.json() as { supplierId: string };
    expect((await request("/api/platform/purchases", { method: "POST", headers, body: JSON.stringify({ businessId: identity.businessId, branchId: identity.branchId, supplierId, idempotencyKey: "finance-purchase-1", items: [{ productId, quantity: 2, unitCostCents: 1000 }] }) })).status).toBe(201);
    const customer = await request("/api/platform/customers", { method: "POST", headers, body: JSON.stringify({ name: "عميل مالي" }) });
    const { customerId } = await customer.json() as { customerId: string };
    const order = await request("/api/platform/orders", { method: "POST", headers, body: JSON.stringify({ businessId: identity.businessId, branchId: identity.branchId, customerId, items: [{ productId, quantity: 1 }] }) });
    expect(order.status).toBe(201);
    const { orderId } = await order.json() as { orderId: string };
    const invoices = await request("/api/platform/invoices", { headers });
    expect(invoices.status).toBe(200);
    const invoiceList = await invoices.json() as { invoices: Array<{ id: string; order_id?: string; source_id: string; total_cents: number }> };
    expect(invoiceList.invoices).toEqual(expect.arrayContaining([expect.objectContaining({ source_id: orderId, total_cents: 2400 })]));
    const invoiceId = invoiceList.invoices.find(invoice => invoice.source_id === orderId)?.id;
    expect(invoiceId).toBeTruthy();
    const invoiceDetail = await request(`/api/platform/invoices/${invoiceId}`, { headers });
    expect(invoiceDetail.status).toBe(200);
    await expect(invoiceDetail.json()).resolves.toMatchObject({ invoice: { source_id: orderId, total_cents: 2400 }, related: { orderId } });
    const expenseBody = { businessId: identity.businessId, branchId: identity.branchId, amountCents: 700, category: "تشغيل", description: "مصروف مالي", idempotencyKey: "finance-expense-1" };
    const expense = await request("/api/platform/expenses", { method: "POST", headers, body: JSON.stringify(expenseBody) });
    expect(expense.status).toBe(201);
    const expenseJson = await expense.json() as { expenseId: string };
    const expenseReplay = await request("/api/platform/expenses", { method: "POST", headers, body: JSON.stringify(expenseBody) });
    expect(expenseReplay.status).toBe(200);
    await expect(expenseReplay.json()).resolves.toMatchObject({ expenseId: expenseJson.expenseId, replay: true });
    const expenseDetail = await request(`/api/platform/expenses/${expenseJson.expenseId}`, { headers });
    expect(expenseDetail.status).toBe(200);
    await expect(expenseDetail.json()).resolves.toMatchObject({ expense: { amount_cents: 700, status: "POSTED" }, ledger: [expect.objectContaining({ debit_cents: 700, credit_cents: 700 })] });
    const balance = await getDataPlane().prepare("SELECT COALESCE(SUM(e.debit_cents), 0) AS debit, COALESCE(SUM(e.credit_cents), 0) AS credit FROM ledger_entries e JOIN ledger_journals j ON j.id = e.journal_id WHERE e.tenant_id = ?").get(identity.tenantId) as { debit: number; credit: number };
    expect(balance.debit).toBe(balance.credit);
    const other = await register("finance-other@example.com", "Finance Other Tenant");
    expect((await request(`/api/platform/invoices/${invoiceId}`, { headers: auth(other) })).status).toBe(404);
    expect((await request(`/api/platform/expenses/${expenseJson.expenseId}`, { headers: auth(other) })).status).toBe(404);
    expect((await request("/api/platform/ledger/journals", { headers: auth(other) })).status).toBe(200);
    const cancel = await request(`/api/platform/expenses/${expenseJson.expenseId}/cancel`, { method: "POST", headers });
    expect(cancel.status).toBe(200);
    await expect((await request(`/api/platform/expenses/${expenseJson.expenseId}`, { headers })).json()).resolves.toMatchObject({ expense: { status: "CANCELLED" }, ledger: [expect.anything(), expect.objectContaining({ reference_type: "EXPENSE_CANCEL", debit_cents: 700, credit_cents: 700 })] });
  });
});
