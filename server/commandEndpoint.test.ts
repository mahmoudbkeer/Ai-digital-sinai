...
async function prepare(headers: Record<string, string> = {}) {
  return fetch(`${baseUrl}/api/commands/prepare`, {
    ...
    body: JSON.stringify({
      sectorId: "retail",
      moduleId: "retail-catalog",
      operationId: "retail-publish",
    }),
  });
}
...
describe("command prepare endpoint authorization", () => {
  beforeAll(async () => {
    ...
    serverProcess.stderr?.on("data", (chunk: Buffer) => {
      serverErrorLogs += chunk.toString();
    });
    await waitForServer();
  });
  ...
  it("logs HTTP errors safely without sensitive headers", async () => {
    const response = await fetch(`${baseUrl}/api/commands/prepare`, {
      ...
      headers: {
        ...
        "x-request-id": "error-request-001",
    ...
    await expect(response.json()).resolves.toMatchObject({
      ...
      requestId: "error-request-001",
    });
    for (
      let attempt = 0;
      attempt < 20 && !serverErrorLogs.includes('"event":"http_error"');
      attempt += 1
    )
  ...
  it("requires an idempotency key after context validation", async () => {
    const signature = signCommandContext(
  ...
  it("replays the same idempotent response and rejects a conflicting context", async () => {
    ...
    const signatureA = signCommandContext(
      { userId: "user-a", workspaceId: "workspace-a" },
      secret
    );
    const first = await prepare({
      "x-command-user": "user-a",
      "x-workspace-id": "workspace-a",
      "x-command-context-signature": signatureA,
      "idempotency-key": key,
    });
    ...
    const signatureB = signCommandContext(
    ...
    const conflict = await prepare({
      ...
      "x-command-context-signature": signatureB,
      "idempotency-key": key,
    });
