...
async function post(payload: string) {
  return fetch(`${baseUrl}/api/payments/webhook`, {
    ...
    headers: {
      ...
      "x-payment-provider": "paymob",
      "x-payment-signature": signed(payload),
    },
    body: payload,
  });
}
...
describe("payment webhook persistence", () => {
  ...
  it("verifies and persists the first event", async () => {
    const response = await post(
      JSON.stringify({ id: "evt-001", event: "payment.updated" })
    );
  ...
  it("returns an idempotent replay and rejects a changed payload", async () => {
    ...
    const conflict = await post(
      JSON.stringify({ id: "evt-001", event: "payment.failed" })
    );
