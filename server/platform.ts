...
const ALL: Permission[] = [
  ...
  "ledger.read", "ledger.manage", "payment.read", "payment.manage", "payment.refund", "subscription.read",
  "subscription.manage", "ai.read", "ai.manage", "notification.read", "notification.manage", "audit.read",
  "admin.manage",
];
const ROLE_PERMISSIONS: Record<Role, Permission[]> = {
  ...
  ACCOUNTANT: ["tenant.read", "business.read", "branch.read", "customer.read", "order.read", "ledger.read", "ledger.manage", "payment.read", "payment.manage", "subscription.read", "audit.read"],
  ...
  MARKETING: ["tenant.read", "business.read", "product.read", "customer.read", "order.read", "ai.read", "ai.manage", "notification.read", "notification.manage"],
...
const ORDER_TRANSITIONS: Record<string, string[]> = {
  ...
  READY: ["OUT_FOR_DELIVERY", "COMPLETED", "CANCELLED"], OUT_FOR_DELIVERY: ["COMPLETED"], COMPLETED: ["REFUNDED"],
  CANCELLED: [], REFUNDED: [],
};
...
export function createPlatformRouter(): Router {
  ...
  router.post("/subscriptions", authenticate, (req: AuthenticatedRequest, res, next) => {
    try {
      ...
      const db = getDatabase();
      const plan = db.prepare("SELECT code, trial_days, active FROM plans WHERE code = ? AND active = 1").get(planCode.trim()) as { code: string; trial_days: number; active: number } | undefined;
      ...
      db.prepare(existing ? "UPDATE subscriptions SET plan_code = ?, status = ?, current_period_start = ?, current_period_end = ?, cancel_at_period_end = 0, updated_at = ? WHERE id = ? AND tenant_id = ?" : "INSERT INTO subscriptions (id, tenant_id, plan_code, status, current_period_start, current_period_end, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)").run(...(existing ? [plan.code, plan.trial_days > 0 ? "TRIALING" : "ACTIVE", started, end, started, subscriptionId, context.tenantId] : [subscriptionId, context.tenantId, plan.code, plan.trial_days > 0 ? "TRIALING" : "ACTIVE", started, end, started, started]));
      recordAudit(db, context, "subscription.activate", "subscription", subscriptionId, req.requestId, { planCode: plan.code, trialDays: plan.trial_days });
      return res.status(existing ? 200 : 201).json({ ok: true, subscriptionId, planCode: plan.code, status: plan.trial_days > 0 ? "TRIALING" : "ACTIVE", currentPeriodEnd: end });
    } catch (error) { next(error); }
  });

  router.get("/subscription", authenticate, (req: AuthenticatedRequest, res, next) => {
    try { const context = currentContext(req); assertScope(context, context.tenantId, "subscription.read"); return res.json({ ok: true, subscription: getDatabase().prepare("SELECT id, plan_code, status, current_period_start, current_period_end, cancel_at_period_end FROM subscriptions WHERE tenant_id = ?").get(context.tenantId) ?? null }); } catch (error) { next(error); }
  });
  ...
  router.post("/deliveries", authenticate, (req: AuthenticatedRequest, res, next) => {
    try {
      ...
      const result = withTransaction(db, () => {
        ...
        const deliveryId = randomUUID(); const timestamp = now(); const state = driverId ? "ASSIGNED" : "PENDING";
  ...
  router.patch("/deliveries/:deliveryId/state", authenticate, (req: AuthenticatedRequest, res, next) => {
    try {
      ...
      const transitions: Record<string, string[]> = { PENDING: ["ASSIGNED", "CANCELLED"], ASSIGNED: ["PICKED_UP", "CANCELLED"], PICKED_UP: ["IN_TRANSIT", "FAILED"], IN_TRANSIT: ["DELIVERED", "FAILED"], DELIVERED: [], FAILED: [], CANCELLED: [] };
  ...
  router.post("/notifications", authenticate, (req: AuthenticatedRequest, res, next) => {
    try {
      ...
      const notificationId = randomUUID(); db.prepare("INSERT INTO notifications (id, tenant_id, user_id, channel, title, body, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)").run(notificationId, context.tenantId, userId, channel, title.trim(), body.trim(), now());
      recordAudit(db, context, "notification.create", "notification", notificationId, req.requestId, { channel, userId });
      return res.status(201).json({ ok: true, notificationId, status: "QUEUED", delivery: channel === "IN_APP" ? "available" : "requires-provider-setup" });
    } catch (error) { next(error); }
  });
